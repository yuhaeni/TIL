...
### 런타임 시점의 세 단계 — "런타임" 용어 정리

"런타임 시점에 결정되는 값이니까 `@Value` 만으로 동적 바인딩이 되겠지?" 라는 오해는 **"런타임"이라는 단어가 너무 넓기 때문**에 생긴다. 한 단계 더 쪼개서 보면 명확해진다.

```
[런타임 전체 = 컴파일 이후 모든 시점]
  ├─ ① 어플리케이션 시작 시점 (Spring 컨테이너 초기화)
  │     → 싱글톤 @Bean 생성, @Value("${...}") 평가
  │     → jobParameters 컨텍스트는 ❌ 아직 없음
  │
  ├─ ② 어플리케이션 실행 중 (요청 처리, 스케줄러 동작 등)
  │
  └─ ③ Job 실행 시점 (Spring Batch Job launch)
        → jobParameters 컨텍스트 생성 ✅
        → @StepScope 빈은 Step 시작 시 비로소 생성됨
        → 이때야 #{jobParameters[...]} 평가 가능
```

**핵심 포인트:**
- `@Value` 는 *빈이 만들어지는 그 순간* 에 *한 번* 평가된다. 그 자체가 "지연 평가" 메커니즘이 아니다.
- 빈이 ① 시점에 만들어지면 → SpEL이 그 시점에 `jobParameters` 를 찾으려다 실패 → `BeanCreationException` → **앱 부팅 실패**.
- **빈 생성 시점 자체를 ③ 시점으로 옮겨주는 것이 `@StepScope` 의 역할.** SpEL 평가가 ③ 시점에 일어나니 그때는 `jobParameters` 가 컨텍스트에 들어와 있다.

> **면접 예상 질문:** "런타임 시점"이라는 표현이 모호한 이유는? `@StepScope` 없이 `@Value("#{jobParameters[...]}")` 만 쓰면 어떤 예외가 어느 시점에 발생하는가?
...
### SpEL `#{...}` vs Property Placeholder `${...}` — 평가 시점과 대상

`@Value` 안에 들어가는 두 표기는 *완전히 다른 메커니즘*이다. 모양만 비슷해서 자주 혼동된다.

| 표기 | 이름 | 평가 대상 | 사용 예 |
|---|---|---|---|
| `${...}` | **Property Placeholder** | 설정 파일/환경변수의 *정적* 값 | `${spring.datasource.url}` |
| `#{...}` | **SpEL (Spring Expression Language)** | 빈 참조, 메서드 호출, 산술/조건 등 *동적 표현식* | `#{jobParameters['targetDate']}` |

**왜 `jobParameters` 는 `${...}` 가 아닌 `#{...}` 인가?**
- `${...}` 는 *설정 파일(`application.yml` 등)에서 미리 정해진 값*을 꺼내는 용도. **정적**.
- `jobParameters` 는 *Job이 launch될 때 동적으로 들어오는 컨텍스트* → 설정 파일에 있을 수 없는 값.
- 따라서 *빈 참조 형태로 컨텍스트에서 꺼내는* SpEL 표현식이 필요.

**SpEL이라서 가능한 표현 예:**
```java
@Value("#{jobParameters['date'] + '-batch'}")     // 문자열 조합
@Value("#{jobParameters['amount'] * 1.1}")        // 산술 연산
@Value("#{jobParameters['mode'] == 'TEST' ? 100 : 1000}") // 조건 분기
@Value("#{T(java.lang.Math).random()}")           // 정적 메서드 호출
```

`${...}` 로는 위 표현이 전부 불가능하다. *설정 키 조회* 만 가능하기 때문.

**자주 헷갈리는 포인트:**
- 둘 다 `@Value` 안에서 쓰니까 *같은 기능의 다른 표기* 라고 오해하기 쉬움 → 사실은 다른 평가기(Placeholder Resolver vs SpEL Parser)가 처리.
- 한 표현식 안에서 같이 쓸 수도 있음: `@Value("#{'${app.prefix}' + jobParameters['date']}")` (`${...}` 가 먼저 치환된 뒤 `#{...}` 가 평가).

> **면접 예상 질문:** `${...}` 와 `#{...}` 의 차이는? `jobParameters` 접근에 SpEL을 써야 하는 이유는?
...
- **"런타임"은 너무 넓은 단어** — ①앱 시작 / ②실행 중 / ③Job 실행, 세 시점으로 쪼개야 Late Binding이 명확해짐. `@StepScope` 없으면 ① 시점에 SpEL 평가 실패 → `BeanCreationException` → 앱 부팅 실패
- **`#{...}` (SpEL) vs `${...}` (Property Placeholder)** 는 다른 메커니즘 — SpEL은 동적 표현식 평가(빈 참조/산술/조건), Property Placeholder는 설정 파일에서 정적 값 추출. `jobParameters` 는 설정 파일에 없는 런타임 컨텍스트이므로 반드시 `#{...}`
