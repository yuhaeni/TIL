...
> 참고: Spring Batch 6부터는 `.chunk(size, transactionManager)` 형태가 deprecated 되고 `.chunk(size).transactionManager(transactionManager)`로 분리됐다. 자세한 변경 사유는 아래 [Spring Batch 6 .chunk() API 변경](#spring-batch-6-chunk-api-변경--chunk와-transactionmanager-분리) 섹션 참고.
...
### JobBuilderFactory vs JobBuilder — Spring Batch 5에서 바뀐 API

예전 강의에서는 `JobBuilderFactory.get(name)` 패턴이 자주 보인다. Spring Batch 5 (Spring Boot 3.0+)부터 이 Factory들이 **`@Deprecated`** 됐고, `JobBuilder` / `StepBuilder`를 직접 생성하는 방식으로 바뀌었다.

**구버전 (Spring Batch 4.x 이하)**:
```kotlin
...
핵심 차이는 **`JobRepository` 의존성이 코드에서 보이느냐 vs 숨어있느냐**다. Factory는 내부에 JobRepository를 가지고 있어 호출부에서는 안 보이지만, JobBuilder는 생성자 인자로 명시적으로 받는다.

| 구분 | JobBuilderFactory (구버전) | JobBuilder (신버전) |
|------|------|------|
| 의존성 | **숨겨짐** | **명시적** |
| 장점 | **구현이 편리** (JobRepository 신경 안 써도 됨) | **유연성↑** (Job마다 다른 JobRepository 주입 가능), 가독성↑ |
| 상태 | `@Deprecated` | 권장 |
| 등장 시점 | Spring Batch 4.x 이하 | Spring Batch 5.0 / Spring Boot 3.0 ~ |

Spring Batch 5는 **편의성을 포기하고 명시성·유연성을 택한 트레이드오프**다. 예를 들어 메인 DB와 별도 메타 DB를 분리해서 Job마다 다른 JobRepository를 쓰고 싶다면 신버전 방식이 훨씬 유리하다.

> **면접 예상 질문:** Spring Batch 5에서 JobBuilderFactory가 deprecated되고 JobBuilder를 직접 생성하는 방식으로 바뀐 이유는 무엇인가? 구버전 Factory 방식의 장점과 신버전의 장점을 각각 트레이드오프 관점에서 설명하라.

---

### Spring Batch 6 .chunk() API 변경 — .chunk()와 .transactionManager() 분리

Spring Batch 6에서 `.chunk(size, transactionManager)` 형태가 **`@Deprecated`** 됐고, Spring Batch 7에서 제거 예정이다. `.chunk(size)`와 `.transactionManager(transactionManager)`로 호출이 분리됐다.
...
```kotlin
StepBuilder(STEP_NAME, jobRepository)
    .chunk<DiaryReminderResult, DiaryReminderMessage>(CHUNK_SIZE)
...
`transactionManager`는 **여전히 필요하다**. 안 넘기면 chunk 단위 트랜잭션 경계가 사라져 commit/rollback이 동작하지 않는다. 단지 **호출 위치가 분리된 것뿐**이다.

**왜 분리했나** — 단일 책임과 API 일관성을 챙기기 위함:

| 관점 | 설명 |
|------|------|
| 단일 책임 | `.chunk()`의 의미는 "청크 크기 설정"인데, 거기에 transactionManager까지 받으면 **하나의 메서드가 두 가지 책임**을 가짐 |
| API 일관성 | 다른 설정들도 `.reader()`, `.processor()`, `.writer()` 처럼 체이닝 메서드로 분리되어 있음. transactionManager만 `.chunk()`에 끼어있던 게 예외적이었음 |
| 가독성 | 신버전 코드는 한 줄씩 읽으면 "청크 크기 설정 → 트랜잭션 매니저 설정 → reader → ..." 의미가 한눈에 드러남 |

이전 섹션 "API 위치의 의미"에서 봤던 **"빈을 받는 위치가 곧 책임"** 원칙이 한 번 더 강화된 변화다. 이제 진짜로 `.chunk()`는 chunk 크기만, `.transactionManager()`는 트랜잭션 매니저만 책임진다.

> **면접 예상 질문:** Spring Batch 6에서 `.chunk(size, transactionManager)`가 deprecated되고 `.chunk(size).transactionManager(transactionManager)`로 분리된 이유는 무엇인가? 단일 책임 / API 일관성 / 가독성 관점에서 설명하라.
...
- Spring Batch 5부터 **`JobBuilderFactory`/`StepBuilderFactory`가 deprecated**, `JobBuilder(name, jobRepository)` / `StepBuilder(name, jobRepository)`로 직접 생성. 구버전은 **편의성**, 신버전은 **명시성·유연성**의 트레이드오프.
- Spring Batch 6부터 **`.chunk(size, transactionManager)` 도 deprecated** → `.chunk(size).transactionManager(tm)`로 호출 분리. `transactionManager`는 여전히 필요하며, 단일 책임·API 일관성·가독성을 위한 변경.
...
- [Configuring a Step (Spring Batch Reference)](https://docs.spring.io/spring-batch/reference/step/chunk-oriented-processing/configuring.html)
