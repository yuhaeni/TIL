...
무거운 매출 스트리밍 쿼리가 고동시성으로 쏟아지면 공용 자원(Hikari 커넥션 풀 10개)을 다 빨아먹어, 다른 평범한 요청까지 커넥션을 못 받고 같이 느려진다. 측정상 **p95 31초** — 메모리는 풀렸지만 요청들이 커넥션 풀 앞에 줄 서서 하염없이 기다린 결과다. 이를 막기 위해 **벌크헤드(Bulkhead)** 로 무거운 요청의 동시 진입 수를 명시적으로 cap 하는 admission control을 적용했다.
